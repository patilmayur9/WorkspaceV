...
public class UserHistory {
	...
	public void getUserList() throws SQLException {
		...
		System.err.println(
				"Note:- Admin can check the User History by Selecting User Name. Data will Show only If Perticular user added Product into cart");
		...
		try {
			...
			ps = con.prepareStatement("Select * from userRegistration;"); // Query for Fetch UserName from
			...
			while (rs.next()) {
				System.out.println("Username= " + rs.getString(8)); // All username will get Printed from
																	// UserRegistration Table

			}

			// Admin will Select the User and Enter the name to display his product produt
			// History
			// As Cart table is created with table name as Username so We can fetch all
			// Product history of that User

			System.out.println("Please Enter the Name of User to Check the Record=>"); // System will ask admin for
																						// UserName
